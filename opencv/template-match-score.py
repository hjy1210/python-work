    ...
    for f in os.listdir(rootdir):
        ...
        if os.path.isfile(fn) and (fn.endswith(".png") or fn.endswith(".jpg")):
            try:
                img = getCvImage(fn, 0)
                s = f+"@"
                for anchor in templateData:
                    lt = anchor["lt"]
                    rb = anchor["rb"]
                    template = templateImage[lt[1]:rb[1], lt[0]:rb[0]]
                    w = template.shape[1]
                    h = template.shape[0]
                    res = perfomeMatch(img, template, method, False)
                    cv.rectangle(img, (res[0][0], res[0][1]),
                                 (res[0][0]+w, res[0][1]+h), 0, 1)
                    if not "children" in anchor.keys():
                        continue
                    for child in anchor["children"]:
                        if "type" in child.keys() and child["type"] == "sequence":
                            ...
                            for ndx in range(child["count"]):
                                cury = res[0][1]-lt[1] + \
                                    (cy-h1//2)+int(ndx*stepy)
                                curx = res[0][0]-lt[0] + \
                                    (cx-w1//2)+int(ndx*stepx)
                                # print(curx,cury,w1,h1)
                                tot = sum(
                                    sum(img[cury:(cury+h1), curx:(curx+w1)] < 128))
                                if tot > 180:
                                    s = s+label[ndx]
                                cv.rectangle(img, (curx, cury),
                                             (curx+w1, cury+h1), 0, 1)
                        if "type" in child.keys() and child["type"] == "rectangle":
                            lty = res[0][1]-lt[1]+child["lt"][1]
                            ltx = res[0][0]-lt[0]+child["lt"][0]
                            rby = res[0][1]-lt[1]+child["rb"][1]
                            rbx = res[0][0]-lt[0]+child["rb"][0]
                            cv.rectangle(img, (ltx, lty), (rbx, rby), 0, 3)
                print(s)
                if display:
